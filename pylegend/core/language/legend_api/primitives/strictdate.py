...
class LegendApiStrictDate(LegendApiDate):
    ...
    def __convert_to_strictdate_expr(
            val: PyLegendUnion[date, "LegendApiStrictDate"]
    ) -> PyLegendExpressionStrictDateReturn:
    ...
    def validate_param_to_be_strictdate(
            param: PyLegendUnion[date, "LegendApiStrictDate"],
            desc: str
    ) -> None:
        if not isinstance(param, (date, LegendApiStrictDate)):
