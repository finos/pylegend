...
class PandasApiPrimitive(PyLegendPrimitive, metaclass=ABCMeta):
    def ascending(self) -> "PandasApiSortInfo":
        val = self.value()
        if isinstance(val, PyLegendColumnExpression):
            return PandasApiSortInfo(column_expr=val, direction=PandasApiSortDirection.ASC)
        else:
    ...
    def descending(self) -> "PandasApiSortInfo":
        val = self.value()
        if isinstance(val, PyLegendColumnExpression):
            return PandasApiSortInfo(column_expr=val, direction=PandasApiSortDirection.DESC)
        else:
