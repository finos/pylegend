...
class AssignFunction(PandasApiAppliedFunction):
    ...
    def to_pure(self, config: FrameToPureConfig) -> str:
        ...
        for col_name, func in self.__col_definitions.items():
            ...
            is_existing_col = col_name in base_cols
            target_col_name = col_name+col_name_suffix if is_existing_col else col_name
            target_col_name = escape_column_name(target_col_name)
            ...
            if isinstance(res, (Series, GroupbySeries)):
                ...
                for expr in sub_expressions:
                    ...
                    if isinstance(applied_func, RankFunction):
                        ...
                        _, window = applied_func.construct_column_expression_and_window_tuples()[0]
                        window_expr = window.to_pure_expression(config)

            if has_window_func:
                assert isinstance(res, (Series, GroupbySeries))
                pure_expr = res.to_pure_expression(config)
                extend = f"->extend({window_expr}, ~{target_col_name}:{generate_pure_lambda('p,w,r', pure_expr)}"
            else:
                res_expr = res if isinstance(res, PyLegendPrimitive) else convert_literal_to_literal_expression(res)
                pure_expr = res_expr.to_pure_expression(config)
                extend = f"->extend({target_col_name}:{generate_pure_lambda('c', pure_expr)})"

            extends.append(extend)

            if is_existing_col:
                project_lambda = generate_pure_lambda("c", f"$c.{escape_column_name(target_col_name)}")
