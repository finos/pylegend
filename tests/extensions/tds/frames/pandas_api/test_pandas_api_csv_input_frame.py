...
class TestPandasApiCsvInputFrame:
    ...
    def test_csv_non_executable_input_frame_creation(self) -> None:
        frame = PandasApiCsvNonExecutableInputTdsFrame(
    ...
    def test_csv_non_executable_input_frame_assign(self) -> None:
        frame = PandasApiCsvNonExecutableInputTdsFrame(
            csv_string=self.test_csv_string
        )
        ...
        expected_pure = (
            ...
            '1,1,A\n'
            '3,1,B#\n'
            '  ->project(~[id:c|$c.id, grp:c|$c.grp, name:c|$c.name, col4:c|(toOne($c.id) + 1)])'
        )

        assert generate_pure_query_and_compile(
            frame.assign(col4=lambda r: r.get_integer('id') + 1),
            FrameToPureConfig(),
