...
class TestGroupByAppliedFunction:
    ...
    def test_group_by_error_on_duplicate_cols(self) -> None:
        ...
        frame: LegacyApiTdsFrame = LegacyApiTableSpecInputFrame(['test_schema', 'test_table'], columns)
        with pytest.raises(ValueError) as r:
            frame.group_by(["col1"], [LegacyApiAggregateSpecification(lambda x: x, lambda y: y, "col1")])  # type: ignore
    ...
    def test_group_by_error_on_incompatible_map_fn(self) -> None:
        ...
        with pytest.raises(TypeError) as r:
            frame.group_by(["col1"], [LegacyApiAggregateSpecification(lambda: 1, lambda y: y, "col3")])  # type: ignore
    ...
    def test_group_by_error_on_map_fn_evaluation(self) -> None:
        ...
        with pytest.raises(RuntimeError) as r:
            frame.group_by(
                ["col1"], [LegacyApiAggregateSpecification(lambda x: x["col5"], lambda y: y, "col3")]  # type: ignore
            )
        assert r.value.args[0] == ("AggregateSpecification at index 0 (0-indexed) incompatible. "
    ...
    def test_group_by_error_on_incompatible_agg_fn(self) -> None:
        ...
        with pytest.raises(TypeError) as r:
            frame.group_by(
                ["col1"],
                [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda: 1, "col3")]  # type: ignore
            )
    ...
    def test_group_by_error_on_agg_fn_evaluation(self) -> None:
        ...
        with pytest.raises(RuntimeError) as r:
            frame.group_by(
                ["col1"],
                [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.unknown(), "col3")]  # type: ignore
            )
        assert r.value.args[0] == ("AggregateSpecification at index 0 (0-indexed) incompatible. "
    ...
    def test_group_by_error_on_agg_fn_non_primitive(self) -> None:
        ...
        with pytest.raises(ValueError) as r:
            frame.group_by(
                ["col1"],
                [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y, "col3")]  # type: ignore
            )
        assert r.value.args[0] == ("AggregateSpecification at index 0 (0-indexed) incompatible. "
    ...
    def test_query_gen_group_by(self) -> None:
        ...
        frame = frame.group_by(
            ["col1"],
            [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.count(), "Count")]
        )
    ...
    def test_query_gen_multi_group_by(self) -> None:
        ...
        frame = frame.group_by(
            ["col1"],
            [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.count(), "Count1")]
        )
        frame = frame.group_by(
            ["col1"],
            [LegacyApiAggregateSpecification(lambda x: x["Count1"], lambda y: y.count(), "Count2")]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_multi_agg(self) -> None:
        ...
        frame = frame.group_by(
            ...
            [
                LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.count(), "Count1"),
                LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.count(), "Count2")
            ]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_distinct_count_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"], lambda y: y.distinct_count(), "Cnt")]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_average_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"], lambda y: y.average(), "Average")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_average_agg_pre_op(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"] + 20, lambda y: y.average(), "Average")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
            "[TdsColumn(Name: col2, Type: String), TdsColumn(Name: Average, Type: Float)]"
        )
    ...
    def test_query_gen_group_by_average_agg_post_op(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"], lambda y: y.average() + 2, "Average")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_integer_max_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"], lambda y: y.max(), "Maximum")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_integer_min_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"], lambda y: y.min(), "Minimum")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_integer_sum_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col2"],
            [LegacyApiAggregateSpecification(lambda x: x["col1"], lambda y: y.sum(), "Sum")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_string_max_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col1"],
            [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.max(), "Maximum")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_string_min_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col1"],
            [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.min(), "Minimum")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_query_gen_group_by_join_strings_agg(self) -> None:
        ...
        frame = frame.group_by(
            ["col1"],
            [LegacyApiAggregateSpecification(lambda x: x["col2"], lambda y: y.join(' '), "Joined")]  # type: ignore
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == (
    ...
    def test_e2e_group_by(self, legend_test_server: PyLegendDict[str, PyLegendUnion[int, ]]) -> None:
        ...
        frame = frame.group_by(
            ["Firm/Legal Name"],
            [
                LegacyApiAggregateSpecification(lambda x: x['First Name'], lambda y: y.count(), 'Employee Count')
            ]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == \
    ...
    def test_e2e_group_by_on_literal(self, legend_test_server: PyLegendDict[str, PyLegendUnion[int, ]]) -> None:
        ...
        frame = frame.group_by(
            [],
            [
                LegacyApiAggregateSpecification(lambda x: 1, lambda y: y.count(), 'Total Employees')
            ]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == \
    ...
    def test_e2e_group_by_multi_grouping_cols(self, legend_test_server: PyLegendDict[str, PyLegendUnion[int, ]]) \
        ...
        frame = frame.group_by(
            ...
            [
                LegacyApiAggregateSpecification(lambda x: x['Last Name'], lambda y: y.count(), 'Employee Count')
            ]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == \
    ...
    def test_e2e_group_by_multi_aggregations(self, legend_test_server: PyLegendDict[str, PyLegendUnion[int, ]]) -> None:
        ...
        frame = frame.group_by(
            ["Firm/Legal Name"],
            [
                LegacyApiAggregateSpecification(lambda x: x['First Name'], lambda y: y.count(), 'Employee Count1'),
                LegacyApiAggregateSpecification(lambda x: x['First Name'], lambda y: y.count(), 'Employee Count2')
            ]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == \
    ...
    def test_e2e_group_by_distinct_count_agg(self, legend_test_server: PyLegendDict[str, PyLegendUnion[int,]]) -> None:
        ...
        frame = frame.group_by(
            ["Firm/Legal Name"],
            [
                LegacyApiAggregateSpecification(lambda x: x['First Name'], lambda y: y.distinct_count(), 'Employee Count')
            ]
        )
        assert "[" + ", ".join([str(c) for c in frame.columns()]) + "]" == \
               "[TdsColumn(Name: Firm/Legal Name, Type: String), TdsColumn(Name: Employee Count, Type: Integer)]"
