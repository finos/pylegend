...
class TestAggregateFunctionAssignment:
    ...
    def test_simple_assignment(self) -> None:
        """Basic: assign an aggregated series to a new column; verify series SQL, frame SQL, and isinstance."""
        columns = [
        ...
        expected_sql_series = '''
            SELECT
                SUM("root".col1_int) AS "col1_int"
            FROM
                test_schema.test_table AS "root"
        '''
        expected_sql_series = dedent(expected_sql_series).strip()
        assert aggregated_series.to_sql_query() == expected_sql_series

        expected_pure_series = '''
            #Table(test_schema.test_table)#
              ->extend(over([~col1_int->ascending()]), ~col1_int__pylegend_olap_column__:{p,w,r | $r.col1_int}:{c | $c->sum()})
              ...
              ->select(~[col1_int])
              ->aggregate(
                ~[col1_int:{r | $r.col1_int}:{c | $c->sum()}]
              )
        '''  # noqa: E501
        expected_pure_series = dedent(expected_pure_series).strip()
        assert generate_pure_query_and_compile(aggregated_series, FrameToPureConfig(), self.legend_client) == expected_pure_series
        ...
        expected_sql_frame = '''
            SELECT
                ...
                SUM("root".col1_int) AS "col1_sum"
            FROM
                test_schema.test_table AS "root"
        '''
        expected_sql_frame = dedent(expected_sql_frame).strip()
        assert frame.to_sql_query() == expected_sql_frame
        ...
        expected_pure_frame = '''
            #Table(test_schema.test_table)#
              ...
              ->project(~[col1_int:c|$c.col1_int, col2_str:c|$c.col2_str, col3_date:c|$c.col3_date, col1_sum:c|$c.col1_int__pylegend_olap_column__])
        '''  # noqa: E501
        expected_pure_frame = dedent(expected_pure_frame).strip()
    ...
    def test_assign_multiple_aggregates_and_overwrite(self) -> None:
        ...
        expected_sql = '''
            SELECT
                ...
                "root".col2 AS "col2",
                "root".col3 AS "col3",
                SUM("root".col1) AS "col1_sum",
                AVG("root".col2) AS "col2_mean",
                COUNT("root".col3) AS "col3_count"
            FROM
                test_schema.test_table AS "root"
        '''
        ...
        expected_pure = '''
            #Table(test_schema.test_table)#
              ->extend(over([~col1->ascending()]), ~col1__pylegend_olap_column__:{p,w,r | $r.col1}:{c | $c->sum()})
              ->project(~[col1:c|$c.col1, col2:c|$c.col2, col3:c|$c.col3, col1_sum:c|$c.col1__pylegend_olap_column__])
              ->extend(over([~col2->ascending()]), ~col2__pylegend_olap_column__:{p,w,r | $r.col2}:{c | $c->average()})
              ->project(~[col1:c|$c.col1, col2:c|$c.col2, col3:c|$c.col3, col1_sum:c|$c.col1_sum, col2_mean:c|$c.col2__pylegend_olap_column__])
              ->extend(over([~col3->ascending()]), ~col3__pylegend_olap_column__:{p,w,r | $r.col3}:{c | $c->count()})
        ...
        # Now overwrite an existing column with an aggregate
        frame["col1"] = frame["col1"].max()

        expected_sql_overwrite = '''
            SELECT
                MAX("root".col1) AS "col1",
                "root".col2 AS "col2",
                "root".col3 AS "col3",
            ...
            FROM
                test_schema.test_table AS "root"
        '''
        expected_sql_overwrite = dedent(expected_sql_overwrite).strip()
        assert frame.to_sql_query() == expected_sql_overwrite

        expected_pure_overwrite = '''
            #Table(test_schema.test_table)#
              ->extend(over([~col1->ascending()]), ~col1__pylegend_olap_column__:{p,w,r | $r.col1}:{c | $c->sum()})
              ->project(~[col1:c|$c.col1, col2:c|$c.col2, col3:c|$c.col3, col1_sum:c|$c.col1__pylegend_olap_column__])
              ->extend(over([~col2->ascending()]), ~col2__pylegend_olap_column__:{p,w,r | $r.col2}:{c | $c->average()})
              ...
              ->extend(over([~col3->ascending()]), ~col3__pylegend_olap_column__:{p,w,r | $r.col3}:{c | $c->count()})
              ->project(~[col1:c|$c.col1, col2:c|$c.col2, col3:c|$c.col3, col1_sum:c|$c.col1_sum, col2_mean:c|$c.col2_mean, col3_count:c|$c.col3__pylegend_olap_column__])
              ->extend(over([~col1->ascending()]), ~col1__pylegend_olap_column__:{p,w,r | $r.col1}:{c | $c->max()})
        ...
        '''  # noqa: E501
        expected_pure_overwrite = dedent(expected_pure_overwrite).strip()
        assert generate_pure_query_and_compile(frame, FrameToPureConfig(), self.legend_client) == expected_pure_overwrite

    def test_assign_aggregate_after_truncate_produces_subquery(self) -> None:
    ...
    def test_assign_aggregate_with_arithmetic(self) -> None:
        """Aggregate result combined with arithmetic (e.g. sum + 42) produces correct SQL."""
        columns = [
            PrimitiveTdsColumn.integer_column("col1"),
            PrimitiveTdsColumn.string_column("col2"),
        ]
        ...
        expected_sql = '''
            SELECT
                "root".col1 AS "col1",
                "root".col2 AS "col2",
                (SUM("root".col1) + 42) AS "col1_sum_plus"
            FROM
                test_schema.test_table AS "root"
        '''
        expected_sql = dedent(expected_sql).strip()
    ...
    def test_assign_aggregate_arithmetic_multiple_ops(self) -> None:
        ...
        expected_sql = '''
            SELECT
                "root".col1 AS "col1",
                "root".col2 AS "col2",
                (SUM("root".col1) - 10) AS "col1_shifted",
                (AVG("root".col2) * 2) AS "col2_scaled"
            FROM
                test_schema.test_table AS "root"
        '''
        expected_sql = dedent(expected_sql).strip()
    ...
    def test_series_sql_unchanged_after_assignment(self) -> None:
        """The aggregated series' own SQL is not mutated by being assigned to a frame."""
        columns = [
            PrimitiveTdsColumn.integer_column("col1"),
            PrimitiveTdsColumn.string_column("col2"),
        ]
        ...
        assert agg_series.to_sql_query() == expected_series_sql

        frame["col1_total"] = agg_series
        assert agg_series.to_sql_query() == expected_series_sql
    ...
    def test_multiple_aggregates_on_same_column(self) -> None:
        ...
        expected_pure = '''
            #Table(test_schema.test_table)#
              ...
              ->extend(over([~val->ascending()]), ~val__pylegend_olap_column__:{p,w,r | $r.val}:{c | $c->min()})
              ->project(~[val:c|$c.val, label:c|$c.label, val_sum:c|$c.val_sum, val_min:c|$c.val__pylegend_olap_column__])
              ->extend(over([~val->ascending()]), ~val__pylegend_olap_column__:{p,w,r | $r.val}:{c | $c->max()})
