...
from pylegend.core.language import LegacyApiTdsRow, LegacyApiPrimitive, LegacyApiInteger


class TestLegacyApiInteger:
    ...
    def __generate_sql_string(self, f: PyLegendCallable[[LegacyApiTdsRow], LegacyApiPrimitive]) -> str:
        ret = f(self.tds_row)
        assert isinstance(ret, LegacyApiInteger)
        return self.db_extension.process_expression(
            ret.to_sql_expression({"t": self.base_query}, self.frame_to_sql_config),
            config=self.sql_to_string_config
        )

    def __generate_sql_string_no_integer_assert(self, f: PyLegendCallable[[LegacyApiTdsRow], LegacyApiPrimitive]) -> str:
        ret = f(self.tds_row)
        return self.db_extension.process_expression(
            ret.to_sql_expression({"t": self.base_query}, self.frame_to_sql_config),
            config=self.sql_to_string_config
