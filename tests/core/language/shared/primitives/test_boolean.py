...
class TestPyLegendBoolean:
    ...
    def test_boolean_cast(self) -> None:
        assert self.__generate_sql_string(lambda x: x.get_boolean("col1").cast(PrimitiveType.String)) == \
               'CAST("root".col1 AS TEXT)'
        assert self.__generate_pure_string(lambda x: x.get_boolean("col1").cast(PrimitiveType.String)) == \
               '$t.col1->cast(@String)'
        assert self.__generate_sql_string(lambda x: x.get_boolean("col1").cast(PrimitiveType.Integer)) == \
               'CAST("root".col1 AS INTEGER)'
        assert self.__generate_pure_string(lambda x: x.get_boolean("col1").cast(PrimitiveType.Integer)) == \
               '$t.col1->cast(@Integer)'
