...
class TestPyLegendInteger:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (operator.or_, "|", "bitOr"),
            (operator.xor, "#", "bitXor"),
            (operator.lshift, "<<", "bitShiftLeft"),
            (operator.rshift, ">>", "bitShiftRight"),
        ],
    )
    def test_integer_bitwise_binary_expr(
            self,
            py_op: PyLegendCallable[
                [PyLegendUnion[int, PyLegendInteger], PyLegendUnion[int, PyLegendInteger]], PyLegendInteger
            ],
            sql_op: str,
            pure_fn: str) -> None:
        assert self.__generate_sql_string(
            lambda x: py_op(x.get_integer("col2"), x.get_integer("col1"))
        ...
        assert self.__generate_sql_string(
            lambda x: py_op(x.get_integer("col2"), 10)
        ...
        assert self.__generate_sql_string(
            lambda x: py_op(10, x.get_integer("col2"))
