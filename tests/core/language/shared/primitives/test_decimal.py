...
class TestPyLegendDecimal:
    ...
    def test_decimal_divide_scaled_type_error(self) -> None:
        with pytest.raises(TypeError):
            self.tds_row.get_decimal("col1").divide(self.tds_row.get_decimal("col2"), "not_int")  # type: ignore[arg-type]
    ...
    def test_decimal_round_type_error(self) -> None:
        with pytest.raises(TypeError):
            self.tds_row.get_decimal("col1").round("not_int")  # type: ignore[arg-type]
...
class TestPyLegendDecimalUnit:
    ...
    def test_number_convert_python_decimal_branch(self) -> None:
        ...
        from decimal import Decimal as PythonDecimal
        result = str((num_row.get_number("n") + PythonDecimal("1.5")).to_pure_expression(self.frame_to_pure_config))
        assert result == '(toOne($t.n) + 1.5D)'
